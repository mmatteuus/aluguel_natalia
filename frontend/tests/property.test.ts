...
describe('property content contract', () => {
  it('keeps the known listing facts and eight-step tour sequence', () => {
  ...
  it('uses all twelve provided property photographs in the guided experience', () => {
    const images = property.rooms.flatMap((room) => room.frames.map((frame) => frame.image));
    expect(images).toHaveLength(12);
    expect(new Set(images).size).toBe(12);
    expect(images).toContain('/images/escada-terreo.avif');
    expect(images).toContain('/images/quarto-02.avif');
